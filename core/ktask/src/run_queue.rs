...
static mut RUN_QUEUES: [MaybeUninit<&'static mut RunQueue>; crate::CPU_NUM] =
    [ARRAY_REPEAT_VALUE; crate::CPU_NUM];
...
fn select_run_queue_index(cpumask: KCpuMask) -> usize {
    ...
    loop {
        let index = RUN_QUEUE_INDEX.fetch_add(1, Ordering::SeqCst) % crate::CPU_NUM;
